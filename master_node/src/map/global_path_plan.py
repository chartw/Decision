    ...
    def node_set(self,mapname):
        with open('./map1/'+mapname+'_node.csv', mode='r') as csv_file:
            ...
            for next_r in csv_reader:
                self.nodelist[next_r['nid']] = node(str(next_r['nid']),float(next_r['nx']),float(next_r['ny']))
    ...
    def path_connect(self,sid, gid):
        ...
        for i in range(len(path)-1):
            ...
            for j in range(len(globals()['Lane{}'.format(path[i]+path[i+1])]['x'])):
                self.path.x.append(globals()['Lane{}'.format(path[i]+path[i+1])]['x'][j])
                self.path.y.append(globals()['Lane{}'.format(path[i]+path[i+1])]['y'][j])
