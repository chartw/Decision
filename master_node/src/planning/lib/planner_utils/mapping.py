from lib.planner_utils.exp_mov_avg import ExpMovAbgFilter
...
class Mapping:
    ...
    def mapping(self, planner, circles):
        ...
        for circle in circles:
            ...
            if id == -1:
                self.map[self.obstacle_cnt] = ExpMovAbgFilter(Point32(x, y, 0))
                self.obstacle_cnt += 1

            # 있으면, 해당 key값의 이동평균 필터에 circle의 절대좌표 (x, y) 삽입
            else:
                self.map[id].emaFilter(Point32(x, y, 0))
