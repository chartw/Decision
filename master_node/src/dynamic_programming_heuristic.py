...
def verify_node(node, obstacle_map, min_x, min_y, max_x, max_y):
    if node.x < min_x:
        ...
        return False
    # print("node", node.x, node.y, "len map list", len(obstacle_map[node.x]))
    if node.x >= len(obstacle_map):
        return False
    if node.y >= len(obstacle_map[node.x]):
        return False
    if obstacle_map[node.x-1][node.y-1]:
        return False
...
def calc_obstacle_map(ox, oy, resolution, vr):
    min_x = int(round(min(ox)))
    min_y = int(round(min(oy)))
    max_x = int(round(max(ox)))
    max_y = int(round(max(oy)))
    # print("######test before")
    x_width = int(1*round(max_x - min_x))
    y_width = int(1*round(max_y - min_y))
    # x_width = 100
    # y_width = 100
    # print("####width", x_width, y_width)
    # print("######test after")
...
def calc_index(node, x_width, x_min, y_min):
    # print(node.y, y_min, x_width, node.x, x_min)
