...
class Controller:
    ...
    def steering_angle(self, cur_x, cur_y, cur_yaw, target_x, target_y):
        ...
        delta = degrees(
            atan2(
                2 * self.control_data["WB"] * sin(radians(alpha)) / self.control_data["look_ahead"],
                1,
            )
        )
        # print('delta:',  delta)
        ...
        if self.first == False:
            ...
            if diff > 0:
                if self.LPP_is_done is 0:
                    ...
                    self.ltidx = 0
                    self.gidx, ox, oy = self.makeObstacleMap(
                        self.control_data["cur_x"],
                        self.control_data["cur_y"],
                        self.control_data["cur_yaw"],
                    )
                    self.lpath_x, self.lpath_y = path_plan(
                        self.control_data["cur_x"],
                        self.control_data["cur_y"],
                        self.control_data["cur_yaw"],
                        self.path_x[self.gidx],
                        self.path_y[self.gidx],
                        self.path_yaw[self.gidx],
                        ox,
                        oy,
                        self.animation_cnt,
                    )
    ...
    def getOdoMsg(self, msg):
        ...
        self.control_data["cur_x"], self.control_data["cur_y"] = (
        ...
        if self.first == False:
            ...
            if abs(self.control_data["cur_x"] - 12) < 7.5:
                ...
                # print()
            # print("after parking")
            self.control_data["steering"] = self.cal_steering(
                self.control_data["cur_x"],
                self.control_data["cur_y"],
                self.control_data["cur_yaw"],
                self.control_data["look_ahead"],
            )
