...
SB_COST = 100000.0  # switch back penalty cost
BACK_COST = 500000.0  # backward penalty cost
STEER_CHANGE_COST = 50000.0  # steer angle change penalty cost
...
def hybrid_a_star_planning(start, goal, ox, oy, xy_resolution, yaw_resolution):
    ...
    pq = []
    # print("what22??", start_node.yaw_index, start_node.x_index, start_node.y_index)
    # print("what22??", config.min_yaw, config.min_y, config.min_x)
    # if calc_index(start_node, config) < 0:
    ...
    while True:
        ...
        # print("나 아나 ㅡㅡ 322")
        # print("updated path", is_updated, final_path)
        if is_updated:
            # print("path found")
            break
        ...
        for neighbor in get_neighbors(current, config, ox, oy,
            ...
            if neighbor_index < 0 or hogyun > 10:
                print("error")
                return -1
...
def path_plan(c_x, c_y, c_yaw, g_x, g_y, g_yaw, ox, oy, animation_cnt):
    ...
    #     oy.append(45)
    # for i in range(1000):
    #     ox.append(25.5+0.01*i)
    #     oy.append(40.0+0.01*i)
    # for i in range(500):
    #     ox.append(19.5-0.01*i)
    #     # ox.append(19.5)
    #     oy.append(37.0+0.01*i)

    # Set Initial parameters
    start = [c_x, c_y, np.deg2rad(c_yaw)]
    # goal = [g_x, g_y, np.deg2rad(g_yaw)]
    goal = [g_x, g_y, np.deg2rad(g_yaw)]
    # goal = [17.1547, 37.1791, np.deg2rad(c_yaw)]
