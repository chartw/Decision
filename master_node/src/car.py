...
WB = 1.56/1.5  # rear to front wheel
W = 1.74/1.5  # width of car
LF = 2.28/1.5  # distance from rear to vehicle front end
LB = 0.72/1.5  # distance from rear to vehicle back end
MAX_STEER = np.deg2rad(28.169)  # [rad] maximum steering angle
# MAX_STEER = np.deg2rad(70.0)
...
def rectangle_check(x, y, yaw, ox, oy):
    ...
    for iox, ioy in zip(ox, oy):
        tx = iox - x
        ty = ioy - y
        # converted_xy = np.stack([tx, ty]).T.dot(rot)
...
def plot_car(x, y, yaw):
    ...
    for rx, ry in zip(VRX, VRY):
        converted_xy = np.stack([rx, ry]).T.dot(rot)
...
def move(x, y, yaw, distance, steer, L=WB):
    ...
    # print("yaw is: ", yaw, "steer is: ", np.rad2deg(steer))
