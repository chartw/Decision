...
def path_plan(c_x, c_y, c_yaw, g_x, g_y, g_yaw):
    print("Start Hybrid A* planning")

    ox, oy = [], [] # lidar, vision에서 받기

    # y=x 변환 해서 yaw 값 받아옴. (시뮬이랑 기가차랑 동일하게 인식해주기.)
    # c_yaw = 360-c_yaw # y축 기준 시계방향으로 + .로 바꿔주고 

    # if c_yaw > 90 and c_yaw < 270:
    #     c_yaw = 90 - c_yaw
    # else:
    #     if c_yaw < 90:
    #         c_yaw = (90 - c_yaw)
    #     else:
    #         c_yaw = 360 - c_yaw + 90 # y=x 대칭해서 hyrid기준으로 인식해준거. .


    # << obstacle !!! ( map 포함) 
    # 장애물= 라이다에서 절대좌표(x,y) 받아와서 문제없음. 차선도 절대좌표(x,y)면 문제없음. 
    # 도로 윤관선 map으로  csv 읽어와서  append 해주자. 

    for i in range(10):
        ox.append(4.43-0.1*i)
        oy.append(7.95)

    # for i in range(10):
    #     ox.append(19.5)
    # #     oy.append(37.0+0.1*i)
    # for i in range(20):
    #     # ox.append(19.5-0.5*i)
    #     ox.append(19.5)
    #     oy.append(37.0+0.5*i)


    # Set Initial parameters
    start = [c_x, c_y, np.deg2rad(c_yaw)] # already changed
    goal = [8, 15, np.deg2rad(60)]        # hybrid 기준으로 생각해서 적어라. 첫도로 y에서 시계방향 30 도 니까 
    ...
    x = path.x_list
    y = path.y_list
    yaw = path.yaw_list # 사실 안씀 ㅎㅎ 만약 얘 받을거면 controller 에서 또 y=x대칭 식 써주고 받아야함. 
...
def main():
    ...
    goal = [50.0, 50.0, np.deg2rad(180.0)]
